...
export const OnboardingPage = () => {
	...
	const blankCanvasButton = 1 === step ? (
		<Button
			variant="outlined"
			color="secondary"
			onClick={ () => setIsBlankKit( true ) }
		>
			{ __( 'Build with blank canvas', 'hello-plus' ) }
		</Button>
	) : null;
		...
		<ThemeProvider colorScheme="auto">
			<Modal open sx={ { zIndex: 100000 } } >
				<Box sx={ {
					...
					{ ! previewKit && ( <TopBarContent onClose={ onClose } actionButton={ blankCanvasButton } sx={ { borderBottom: '1px solid var(--divider-divider, rgba(0, 0, 0, 0.12))', mb: 4 } } iconSize="small" /> ) }
					...
					{ 2 === step && ! isLoading && ! previewKit && (
						<ReadyToGo
							...
							title={ isBlankKit ? titleBlank : readyTitle }
							description={ isBlankKit ? descriptionBlank : readyDescription }
							viewSite={ isBlankKit ? viewSiteBlank : viewSite }
							customizeSite={ isBlankKit ? customizeSiteBlank : customizeSite }
						/>
					) }
