...
class Onboarding_Settings {
	...
	public function get_kits() {
		$transient_name = $this->get_kits_transient_name();
		$kits = get_transient( $transient_name );
		...
		$args = [
			...
			'hello_plus_version' => HELLO_PLUS_VERSION,
		...
		try {
			...
			set_transient( $transient_name, $sorted_kits, HOUR_IN_SECONDS );
		} catch ( \Exception $e ) {
			return [];
		}
	...
	private function get_kits_transient_name(): string {
		return self::KITS_TRANSIENT . '_' . HELLO_PLUS_VERSION;
	}
	...
	public function get_onboarding_settings() {
		...
		$blank_canvas_nonce = wp_create_nonce( 'blank-canvas-option' );
		...
		$ready_to_go_text = apply_filters(
			...
			[
				...
				'titleBlank' => __( 'You\'re ready to build your website', 'hello-plus' ),
				'descriptionBlank' => __(
					'You\'ve chosen to build from scratch. For a faster way to start, go back to the library for a website template that best suits your business.',
					'hello-plus'
				),
				'viewSiteBlank' => __( 'Back to library', 'hello-plus' ),
				'customizeSiteBlank' => __( 'Start building', 'hello-plus' ),
			]
		);
		...
		return rest_ensure_response(
			[
				'settings' => [
					'nonce' => $nonce,
					'blankCanvasNonce' => $blank_canvas_nonce,
					...
					'blankCanvasRedirectUrl' => add_query_arg(
						[ 'action' => Blank_Canvas_Page_Creator::ACTION_PARAM ],
						self_admin_url( 'admin.php' )
					),
					'kits' => $kits,
					'applyKitBaseUrl' => version_compare( defined( 'ELEMENTOR_VERSION' ) ? ELEMENTOR_VERSION : '0', self::MIN_ELEMENTOR_CUSTOMIZATION_VERSION, '>=' ) ?
						self_admin_url( 'admin.php?page=elementor-app#import-customization' ) :
						self_admin_url( 'admin.php?page=elementor-app#import/process' ),
