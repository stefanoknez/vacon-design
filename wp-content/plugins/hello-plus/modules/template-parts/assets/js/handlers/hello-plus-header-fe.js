export default class HelloPlusHeaderHandler extends elementorModules.frontend.handlers.Base {
	...
	applyBodyPadding() {
		...
		const floatingBars = this.elements.floatingBars;
		const hasShapeTypeFloat = this.elements.main.classList.contains( 'shape-type-float' );
		...
		if ( floatingBars ) {
			...
			if ( none !== this.getDataScrollBehavior() ) {
				if ( ! floatingBars.classList.contains( 'is-sticky' ) && ! floatingBars.classList.contains( 'is-hidden' ) ) {
					...
					const paddingTop = hasShapeTypeFloat ? floatingBarsHeight : mainHeight + floatingBarsHeight;
					document.documentElement.style.setProperty( '--ehp-body-padding-top', `${ paddingTop }px` );
				}
			}
		} else {
			const paddingTop = hasShapeTypeFloat ? 0 : mainHeight;
			document.documentElement.style.setProperty( '--ehp-body-padding-top', `${ paddingTop }px` );
		}
	}
	...
	checkInitialScrollPosition() {
		...
		const checkWpAdminBarState = () => {
			if ( ! this.hasFirstScrollOccurred && this.elements.wpAdminBar ) {
				this.handleFirstScroll();
			}
		};
