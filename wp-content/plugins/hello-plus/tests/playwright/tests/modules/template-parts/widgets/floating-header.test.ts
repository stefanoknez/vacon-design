...
import { ensureHeaderExists, getHeadingsHtmlContent, updateHeaderStickyBehavior, updateHeaderConfiguration, deactivateAllFloatingBars } from './ehp-header.helper';
...
test.describe( 'Hello Plus Floating Header', () => {
	test.beforeEach( async ( { browser, apiRequests, storageState }, testInfo ) => {
		...
		await deactivateAllFloatingBars( globalPage );
	} );
	...
	test( 'Hello Plus Floating Header', async () => {
		...
		await test.step( 'Test page with Hello Plus floating header', async () => {
		...
		await test.step( 'Header sticky behavior (advanced tab) test', async () => {
			...
			for ( const behavior of behaviors ) {
				await test.step( `Test sticky behavior: ${ behavior }`, async () => {
					...
					const screenshotName = `floating-header-sticky-${ behavior }-initial.png`;
					...
					const screenshotNameScrolled = `floating-header-sticky-${ behavior }-scrolled.png`;
					...
					if ( 'scroll-up' === behavior ) {
						...
						const screenshotNameScrolledUp = `floating-header-sticky-${ behavior }-scrolled-up.png`;
