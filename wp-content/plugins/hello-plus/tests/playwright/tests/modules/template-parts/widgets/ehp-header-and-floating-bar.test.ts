...
import { ensureHeaderExists, importFloatingBar, setFloatingBarAsEntireSite, getHeadingsHtmlContent, updateHeaderStickyBehavior } from './ehp-header.helper';
...
test.describe( 'Hello Plus Header with Floating Bar', () => {
	...
	test( 'Hello Plus Header with Floating Bar', async () => {
		...
		await test.step( 'Test page with floating bar and Hello Plus header', async () => {
		...
		await test.step( 'Header sticky behavior (advanced tab) test', async () => {
			...
			for ( const behavior of behaviors ) {
				await test.step( `Test sticky behavior: ${ behavior }`, async () => {
					...
					await expect.soft( pageEditor.page ).toHaveScreenshot( `header-sticky-${ behavior }-initial.png`, screenshotOptions );

					await pageEditor.page.mouse.wheel( 0, 90 );
					await pageEditor.page.waitForTimeout( 1000 );

					await expect.soft( pageEditor.page ).toHaveScreenshot( `header-sticky-${ behavior }-scrolled.png`, screenshotOptions );

					if ( 'scroll-up' === behavior ) {
						await pageEditor.page.mouse.wheel( 0, -300 );
						await pageEditor.page.waitForTimeout( 800 );
						await expect.soft( pageEditor.page ).toHaveScreenshot( `header-sticky-${ behavior }-scrolled-up.png`, screenshotOptions );
					}
				} );
			}
		} );
