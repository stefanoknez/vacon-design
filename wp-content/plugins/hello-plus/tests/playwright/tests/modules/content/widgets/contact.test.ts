...
import { setupNoticeRemoval } from '../../../../helpers/removeProNotice';
...
test.beforeEach( async ( { browser, apiRequests }, testInfo ) => {
	...
	await context.addInitScript( setupNoticeRemoval() );
...
test( 'Contact widget can be added to the page', async () => {
	...
	const mapMask = previewFrame.locator( '.ehp-contact__map' );
...
for ( let loopIndex = 0; loopIndex < 3; loopIndex++ ) {
	test( `Contact randomized configuration test ${ loopIndex + 1 }`, async () => {
		...
		const editorMapMask = previewFrame.locator( '.ehp-contact__map' );
		await editor.togglePreviewMode();
