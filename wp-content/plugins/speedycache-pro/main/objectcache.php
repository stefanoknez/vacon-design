...
class ObjectCache{
	...
	static $conf_file = WP_CONTENT_DIR . '/speedycache-config/.speedycache-object.php';
	static $conf;
	static $instance;
	static $persistent;
	static $async_flush;
...
$conf_file = WP_CONTENT_DIR . "/speedycache-config/.speedycache-object.php";
$lib_file = $plugin_dir . "/main/object-cache-lib.php";


if(file_exists($plugin_dir) && file_exists($conf_file) && file_exists($lib_file)){
	$spdf_config = file($conf_file);
	if(empty($spdf_config) || !is_array($spdf_config)){
		return;
	}

	unset($spdf_config[0]);
	$spdf_config = implode("", $spdf_config);
	$spdf_config = json_decode($spdf_config, true);
	
	if (empty($spdf_config) || empty($spdf_config["enable"]) || ! SPEEDYCACHE_OBJECT_CACHE || (empty($spdf_config["admin"]) && defined("WP_ADMIN"))){
