...
class Image{
	...
	static function optimized_file_count(){
		...
		$img_stats = get_option('speedycache_optm_img_stats');
		...
		if($result->optimized && $result->optimized > 0){
			update_option('speedycache_optm_img_stats', $result->optimized, false);
	...
	static function optimize_single($id = null){
		...
		foreach($speedycache->image['images'] as $key => $value){
			...
			if(!empty($res['success'])){
				...
				if(in_array($res['error_code'] , array(2, 6, 7, 11, 19, 20, 22, 23))){
					return array(self::get_error_text($res['error_code']), 'error');
					break;
				}
		...
		delete_option('speedycache_optm_img_stats'); // Deleting the cached stat
	...
	static function statics(){
		$res = ['total_image_number' => 0, 'error' => 0, 'optimized' => 0, 'uncompressed' => 0, 'reduction' => 0, 'percent' => 0];
	...
	?>
		...
		<div class="speedycache-block">
			<div class="speedycache-img-stats speedycache-admin-row">
				...
				<div class="speedycache-is-block speedycache-img-count" style="position:relative;">
					<div>
					...
					<div class="speedycache-img-stat-update-status">Updating Stats <span class="dashicons dashicons-image-rotate"></span></div>
				</div>
			</div>
		</div>
